...
var Counter = function(options){
  ...
  return {
    ...
    display: function( stateOverride ){
      return states[currentState] || stateOverride;
    },
    displayBackspace: function( stateOverride ){
      return Array(states[stateOverride || currentState].length+1).join('\b');
    },
  };
      ...
      Script.ajaxRequest( 'GET', pkgURL, function(res){
        ...
        res.on( 'data', function(data){
          if( !(++indicatorCounter % triggerInterval) ){
            process.stdout.write( Counter.displayBackspace() );
            Counter.advance();
